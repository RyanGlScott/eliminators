...
elimDecision :: forall {m} a (p :: Decision a -> @m Type) (d :: Decision a).
                Sing d
...
type SDecision :: Decision a -> Type
...
instance SingKind a => SingKind (Decision a) where
  type Demote (Decision a) = Decision (Demote a)
...
type NatEqConsequencesBase m = ElimNat (Const Type) m () ConstVoidNat

type NatEqConsequencesStep :: forall (m :: Nat) -> Const (Nat -> Type) m
                           -> Nat -> Const Type (S m)
type NatEqConsequencesStep m r n = ElimNat (Const Type) n Void (EqSameNat m)

type ListEqConsequencesBase :: [e] -> Type
type ListEqConsequencesBase ys = ElimList (Const Type) ys () ConstVoidList

type ListEqConsequencesStep :: forall e. forall (x :: e) (xs :: [e])
                            -> Const ([e] -> Type) xs -> [e] -> Const Type (x:xs)
type ListEqConsequencesStep x xs r ys = ElimList (Const Type) ys Void (EqSameList x xs)

type NatEqConsequences :: Nat -> Nat -> Type
type NatEqConsequences n m =
  ElimNat (Const (Nat -> Type)) n
          NatEqConsequencesBase
          NatEqConsequencesStep m
...
type ListEqConsequences (xs :: [e]) (ys :: [e]) =
  ElimList (Const ([e] -> Type)) xs
           ListEqConsequencesBase
           ListEqConsequencesStep ys
